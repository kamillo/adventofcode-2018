...
input.split(separator: "\n").map{String($0)}.sorted().forEach { (line) in
    if let dateString = line.slice(from: "[", to: "]") {
        if let date = dateFormatter.date(from: dateString) {
            var minutes = Calendar.current.dateComponents([.minute], from: date).minute ?? 0
            if (line.contains("Guard")) {
                guardId = line.slice(from: "#", to: " ") ?? ""
                if (!guardId.isEmpty) {
                    print("ID \(guardId)")
                    if (guards[guardId] == nil) {
                        guards[guardId] = Array(repeating: 0, count: 60)
                    }
                    minutes = 0
                }
                
            } else if (line.contains("wakes")) {
                print("\(guardId) -- \(timePointer, minutes)")
                
                for i in timePointer..<minutes {
                    guards[guardId]?[i] += 1
                }
            }
...
let mostSleepingGuardKey = guards.mapValues { $0.reduce(0, +) }.max { a, b in a.value < b.value }!.key
let mostSleepingGuard = guards[mostSleepingGuardKey]
let mostSleepingMinute = mostSleepingGuard!.indices.filter { mostSleepingGuard!.max() == mostSleepingGuard![$0] }

print(mostSleepingGuard!)
print(mostSleepingMinute)
print(Int(mostSleepingGuardKey)! * mostSleepingMinute[0])
