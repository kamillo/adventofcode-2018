...
final class AnimatedWaveView: UIView {
    ...
    private let baseRect = CGRect(x: 0, y: 0, width: 2, height: 2)
    private var points: [[Int]] = []
    private var iter = 0
    
    convenience init(frame: CGRect, points: [[Int]]) {
    ...
    public func makeWaves() {
        DispatchQueue.main.async {
            Timer.scheduledTimer(timeInterval: 1, target: self, selector:
                #selector(self.animatePoints), userInfo: nil, repeats: true)
        }
    }
    
    private func buildPoint(position: CGPoint) -> CAShapeLayer {
        let circlePath = UIBezierPath(ovalIn: baseRect)
        let pointLayer = CAShapeLayer()
        pointLayer.bounds = baseRect
        pointLayer.frame = baseRect
    ...
    public func drawPoints() {
        for i in points {
            let waveLayer = self.buildPoint(position: CGPoint(x: i[0], y: i[1]))
            self.layer.addSublayer(waveLayer)
        }
    }
    
    @objc public func animatePoints() {
        ...
        UIView.animate(withDuration: 2.0) {
            for layer in (self.layer.sublayers?.indices)! {
                self.layer.sublayers?[layer].position = CGPoint(x: self.points[layer][0], y:self.points[layer][1])
            }
        }
...
var points:[[Int]] = input.split(separator: "\n").reduce(into: []) { (result, line) in
    let point = String(line).slice(from: "position=<", to: ">")!.split(separator: ",").map { Int($0.trimmingCharacters(in: CharacterSet.whitespaces))! }
    let vel = String(line).slice(from: "velocity=<", to: ">")!.split(separator: ",").map { Int($0.trimmingCharacters(in: CharacterSet.whitespaces))! }
    result.append(point + vel)
}


var maxX = points.compactMap { $0[0] }.max()!
var minX = points.compactMap { $0[0] }.min()!
var maxY = points.compactMap { $0[1] }.max()!
var minY = points.compactMap { $0[1] }.min()!
print(minX, maxX, minY, maxY)

//points.indices.map {
//    print((Double(points[$0][2]) / Double(maxX - minX)))
//    points[$0][0] = Int(600 * (Double(points[$0][0] - minX) / Double(maxX - minX)))
//    points[$0][1] = Int(600 * (Double(points[$0][1] - minY) / Double(maxY - minY)))
////    points[$0][2] = Int(600 * (Double(points[$0][2]) / Double(maxX - minX)))
////    points[$0][3] = Int(600 * (Double(points[$0][3]) / Double(maxY - minY)))
//}
...
while maxX - minX > 600 {
    ...
    //    maxY = points.compactMap { $0[1] }.max()!
    //    minY = points.compactMap { $0[1] }.min()!

    iter += 1
    if iter % 300 == 0 {
        
        maxX = points.compactMap { $0[0] }.max()!
        minX = points.compactMap { $0[0] }.min()!
        print(minX, maxX)
    }
}
...
let view = AnimatedWaveView(frame: CGRect(x: 0, y: 0, width: 600, height: 600), points:points)
